...
def client(mock_store):
    api = weather.app.create_app(mock_store)
...
def test_list_images(client):
    doc = {
        'images': [
            {
                'href': '/resources/1eaf6ef1-7f2d-4ecc-a8d5-6e8adba7cc0e.png'
            }
        ]
    }
...
def test_post_image(client, mock_store):
    ...
    assert response.headers['location'] == '/resources/{}'.format(file_name)
...
def test_saving_image(monkeypatch):
    ...
    store = weather.resources.ImageStore(
        storage_path,
